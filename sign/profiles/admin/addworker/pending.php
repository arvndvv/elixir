...
$sql="SELECT * FROM worker WHERE `approved`=0";
    ...
    $sql="SELECT * FROM worker WHERE `approved`=0 AND `name` LIKE '%".$fil."%'";
}
...
while($row=mysqli_fetch_assoc($result)){
   ...
   <center><a href="#" ><button class="mybutton clor"  style="width:120px;" onClick="reject(&apos;'.$row["email"].'&apos;)">Reject</button></a>
   <form action="../addworker/forms.php" method="POST" target="_blank" class="inline">
   <input type="hidden" value="'.$row["email"].'" name="email"></input> 
   <input type="submit" class="mybutton" value="Show details"></input></form></center>
   </div>
 </div>';
}
